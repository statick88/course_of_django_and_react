...
En python la sintaxis es muy sencilla, se basa en la indentación, es decir, en la forma en que se escriben los bloques de código.
...
if 5 > 2: # <1>
  print("Cinco es mayor que dos") # <1>
```
1. En este caso, el `:` indica que se va a escribir un bloque de código, en este caso, el bloque de código que se ejecutará si la condición es verdadera.
...
1. En este caso, se define una función anónima que recibe dos parámetros `x` y `y` y retorna la suma de `x` y `y`.
...
1. En este caso, se define una clase llamada `Persona`.
2. La clase tiene un método llamado `__init__` que recibe tres parámetros: `self`, `nombre` y `edad`.
3. El método `__init__` asigna el valor del parámetro `nombre` a la propiedad `nombre` de la clase.
4. El método `__init__` asigna el valor del parámetro `edad` a la propiedad `edad` de la clase.
...
A continuación se sugiere realiza 
